...
このメソッドパラメータが設定されている場合はWHERE～ORDER BY句までがすべて置き換わります。
...
２　orderbyメソッドパラメータ
order by句を記述します。
whereメソッドパラメータにorder by句が存在しない、もしくはwhereメソッドパラメータそのものがない場合に設定されます。
自動生成されたSQL文の文末にorderbyメソッドパラメータに記述された文字列を追加します。
このメソッドパラメータにはorder by句がなければ「 ORDER BY 」を追加するように記述されているため、
ORDER BY句を省略しても構いません。

３　columnsメソッドパラメータ
SELECT文の「SELECT xxx FROM yyy」でいう結果を出力するカラム名を指定するメソッドパタメータです。
これが設定されているSQL文では最優先されます。
基本的にSELECT文は全カラム出力（*句）にしているため、
SQL文の出力を抑えたり、別名を割り当てたいときに利用します。
